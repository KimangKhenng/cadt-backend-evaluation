...
async function loadResults() {
  const success = await raceResultService.loadFromFile(
    "./data/raceScores.json"
  );
  if (success) {
    ...
    const time = raceResultService.getTimeForParticipant(
      "participant1",
      "swim"
    );
    console.log(
      "Participant1 swim time:",
      time ? time.toString() : "Not found"
    ); // Expected: 2m 30s
  } else {
