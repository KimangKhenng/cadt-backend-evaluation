...
export class RaceResultsService {
  ...
  saveToFile(filePath) {
    ...
    const plainResults = this._raceResults.map(result => ({
      participantId: result.particicpantId,
      sport: result.sport,
  ...
  getTimeForParticipant(participantId, sport) {
       ...
       const found = this._raceResults.find(
        r => r.particicpantId === participantId && r.sport === sport
       );
  ...
  getTotalTimeForParticipant(participantId) {
        ...
        const results = this._raceResults.filter(
          r => r.particicpantId === participantId
        );

        if (results.length === 0) return new Duration(0);

        return results.reduce((total, curr) => total.plus(curr.duration), new Duration(0));
  }
}
