...
export class RaceResultsService {
  ...
  addRaceResult(result) {
    ...
    if (!(result instanceof RaceResult)) {
      throw new TypeError("result must be an instance of RaceResult");
    }
      ...
      this._raceResults = parsedData.map(
        (item) =>
          new RaceResult(
            item.participantId,
            item.sportType,
            Duration.fromMinutesAndSeconds(
              Math.floor(item.duration._totalSeconds / 60),
              item.duration._totalSeconds % 60
            )
          )
      );
      return true;
